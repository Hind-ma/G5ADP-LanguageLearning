...
def install_libraries_from_file(file_path, record_dependencies=True):
    with open(file_path, 'r') as file:
        for line in file:
            install_library_from_line(line.strip())
            if record_dependencies and file_path != DEPENDENCY_FILE:
                record_dependency(line.strip())

def install_library_from_line(line):
    try:
